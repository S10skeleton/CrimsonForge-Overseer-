...
const FP_ACCENT = '#4ACCFE'

export default function ForgePilotFeedbackTab() {
  ...
  const load = () => {
    setLoading(true)
    api.fp.feedback().then(setItems).finally(() => setLoading(false))
  }
  useEffect(load, [])
  ...
  const cycleStatus = async (id: string, current: string) => {
    ...
    await api.fp.updateFeedback(id, next)
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28, flexWrap: 'wrap', gap: 12 }}>
        <h1 style={{ fontFamily: 'Orbitron', fontWeight: 900, fontSize: 22, letterSpacing: 4 }} className="grad">FORGEPILOT FEEDBACK</h1>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {(['all','bug','suggestion','praise'] as const).map(f => (
            <button
              ...
              style={filter === f ? { borderColor: FP_ACCENT, color: FP_ACCENT, background: 'rgba(74,204,254,.1)' } : {}}
            >
            ...
            return (
              <div key={item.id} className="card">
                ...
                <div style={{ display: 'flex', gap: 14, fontSize: 11, color: 'var(--dim)', flexWrap: 'wrap' }}>
                  ...
                  {item.page && <span style={{ fontFamily: 'Share Tech Mono' }}>{item.page}</span>}
                </div>
              </div>
            )
          })}
